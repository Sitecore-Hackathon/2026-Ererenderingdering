...
export const getDomainTest: ContentItem = {
  name: "getDomain",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (!Array.isArray(domains) || domains.length === 0) throw new Error('No domains available');
  let domainName = domains[0].name || domains[0].domainName || domains[0];
  if (typeof domainName === 'object') domainName = 'sitecore';
  const domain = await sc.Security.getDomain(domainName);
  if (!domain) throw new Error('domain is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getDomain (' + duration + 'ms)');`,
  },
};
