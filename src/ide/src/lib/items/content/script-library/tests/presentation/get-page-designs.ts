...
export const getPageDesignsTest: ContentItem = {
  name: "getPageDesigns",
  template: "jsScript",
  fields: {
    "__Display name": "Get Page Designs",
    Script: `// Test: Presentation > getPageDesigns
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const designs = await sc.Presentation.getPageDesigns(siteName);
  if (!designs) throw new Error('page designs is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getPageDesigns (' + duration + 'ms)');`,
  },
};
