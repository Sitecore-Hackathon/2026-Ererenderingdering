...
export const populateManagedSchemaTest: ContentItem = {
...
try {
  ...
  try {
    const result = await sc.Indexes.populateManagedSchema([indexName]);
    if (!result) throw new Error('populateManagedSchema returned null');
  } catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>populateManagedSchema</strong>
...
\`);
print(status + ' populateManagedSchema (' + duration + 'ms)');`,
  },
};
