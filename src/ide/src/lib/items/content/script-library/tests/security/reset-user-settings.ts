...
export const resetUserSettingsTest: ContentItem = {
...
try {
  ...
  try {
    ...
    const result = await sc.Security.resetUserSettings(userName);
    if (!result) throw new Error('reset user settings result is null');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>resetUserSettings</strong>
...
\`);
print(status + ' resetUserSettings (' + duration + 'ms)');`,
  },
};
