...
export const getItemChildrenTest: ContentItem = {
...
try {
  ...
  if (!Array.isArray(children)) throw new Error('children should be array');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getItemChildren (' + duration + 'ms)');`,
  },
};
