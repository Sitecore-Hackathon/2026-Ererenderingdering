...
export const getSitesTest: ContentItem = {
  name: "getSites",
  template: "jsScript",
  fields: {
    "__Display name": "Get Sites",
    Script: `// Test: Sites > getSites
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!sites) throw new Error('sites is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getSites (' + duration + 'ms)');`,
  },
};
