...
export const isJobQueuedTest: ContentItem = {
  name: "isJobQueued",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (result !== false) throw new Error('nonexistent job should not be queued');
  passed = true;
} catch (e) {
...
\`);
print(status + ' isJobQueued (' + duration + 'ms)');`,
  },
};
