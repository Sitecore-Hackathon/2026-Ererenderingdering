...
export const getTenantTemplatesTest: ContentItem = {
  name: "getTenantTemplates",
  template: "jsScript",
  fields: {
    "__Display name": "Get Tenant Templates",
    Script: `// Test: Templates > getTenantTemplates
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const templates = await sc.Templates.getTenantTemplates(siteName);
  if (!templates) throw new Error('tenant templates is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getTenantTemplates</strong>
...
\`);
print(status + ' getTenantTemplates (' + duration + 'ms)');`,
  },
};
