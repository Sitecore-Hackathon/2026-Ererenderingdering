...
export const getUsersTest: ContentItem = {
  name: "getUsers",
  template: "jsScript",
  fields: {
    "__Display name": "Get Users",
    Script: `// Test: Security > getUsers
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!users) throw new Error('users result is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getUsers (' + duration + 'ms)');`,
  },
};
