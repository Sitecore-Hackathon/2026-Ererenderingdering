...
export const getArchivedItemTest: ContentItem = {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_getarch';
  ...
  try {
    ...
    try {
      const archived = await sc.Languages.getArchivedItem(archiveResult.archiveItemId);
      if (!archived) throw new Error('archived item is null');
    } finally {
...
\`);
print(status + ' getArchivedItem (' + duration + 'ms)');`,
  },
};
