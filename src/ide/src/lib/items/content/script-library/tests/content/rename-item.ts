...
export const renameItemTest: ContentItem = {
  name: "renameItem",
  template: "jsScript",
  fields: {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_rename';
  const item = await sc.createItem('/sitecore/content', SAMPLE_TEMPLATE, name);
  try {
    if (!item || !item.itemId) throw new Error('setup failed');
    const newName = '__sctest_' + Date.now() + '_renamed';
    const renamed = await sc.renameItem(item.itemId, newName);
    if (!renamed) throw new Error('renameItem returned null');
    if (renamed.name !== newName) throw new Error('name mismatch: expected ' + newName + ', got ' + renamed.name);
  } finally {
...
\`);
print(status + ' renameItem (' + duration + 'ms)');`,
  },
};
