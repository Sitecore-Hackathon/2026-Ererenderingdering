...
export const deleteArchivedVersionTest: ContentItem = {
  name: "deleteArchivedVersion",
  template: "jsScript",
  fields: {
    "__Display name": "Delete Archived Version",
    Script: `// Test: Languages > deleteArchivedVersion
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_delarchver';
  ...
  try {
    ...
    if (!archiveResult || !archiveResult.archiveVersionId) throw new Error('archive version failed');
    const result = await sc.Languages.deleteArchivedVersion(archiveResult.archiveVersionId);
    if (!result) throw new Error('delete archived version result is null');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>deleteArchivedVersion</strong>
...
\`);
print(status + ' deleteArchivedVersion (' + duration + 'ms)');`,
  },
};
