...
export const getUserTest: ContentItem = {
  name: "getUser",
  template: "jsScript",
  fields: {
    "__Display name": "Get User",
    Script: `// Test: Security > getUser
const t0 = Date.now();
let passed = false;
let error = null;

try {
  const current = await sc.Security.getCurrentUser();
  if (!current || !current.name) throw new Error('Cannot get current user name');
  const user = await sc.Security.getUser(current.name);
  if (!user) throw new Error('user is null');
  if (!user.name) throw new Error('user missing name');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getUser (' + duration + 'ms)');`,
  },
};
