...
export const publishLanguageSpecificTest: ContentItem = {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_publs';
  ...
  try {
    ...
    const result = await sc.Publishing.publishLanguageSpecificItems({ itemsToPublish: [{ id: item.itemId, languages: ['en'] }], targetDatabases: [targetDb], publishItemMode: 'SMART' });
    if (!result) throw new Error('publish language specific items result is null');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>publishLanguageSpecific</strong>
...
\`);
print(status + ' publishLanguageSpecific (' + duration + 'ms)');`,
  },
};
