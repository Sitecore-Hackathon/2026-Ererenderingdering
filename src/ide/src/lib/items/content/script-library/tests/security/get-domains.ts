...
export const getDomainsTest: ContentItem = {
  name: "getDomains",
  template: "jsScript",
  fields: {
    "__Display name": "Get Domains",
    Script: `// Test: Security > getDomains
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!Array.isArray(domains)) throw new Error('domains should be array');
  if (domains.length === 0) throw new Error('should have domains');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getDomains (' + duration + 'ms)');`,
  },
};
