...
export const getPageBranchesRootsTest: ContentItem = {
...
try {
  ...
  const roots = await sc.Presentation.getPageBranchesRoots(siteName);
  if (!roots) throw new Error('page branches roots is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getPageBranchesRoots</strong>
...
\`);
print(status + ' getPageBranchesRoots (' + duration + 'ms)');`,
  },
};
