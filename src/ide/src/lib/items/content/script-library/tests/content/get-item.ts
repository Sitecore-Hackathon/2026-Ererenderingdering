...
export const getItemTest: ContentItem = {
  name: "getItem",
  template: "jsScript",
  fields: {
    "__Display name": "Get Item",
    Script: `// Test: Content > getItem (by path)
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (item.name !== 'content') throw new Error('Expected name "content", got "' + item.name + '"');
...
\`);
print(status + ' getItem (' + duration + 'ms)');`,
  },
};
