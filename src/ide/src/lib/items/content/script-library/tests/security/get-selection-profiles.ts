...
export const getSelectionProfilesTest: ContentItem = {
  name: "getSelectionProfiles",
  template: "jsScript",
  fields: {
    "__Display name": "Get Selection Profiles",
    Script: `// Test: Security > getSelectionProfiles
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!profiles) throw new Error('selection profiles is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getSelectionProfiles</strong>
...
\`);
print(status + ' getSelectionProfiles (' + duration + 'ms)');`,
  },
};
