...
export const getIndexTest: ContentItem = {
  name: "getIndex",
  template: "jsScript",
  fields: {
    "__Display name": "Get Index",
    Script: `// Test: Indexes > getIndex
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const index = await sc.Indexes.getIndex(indexName);
...
\`);
print(status + ' getIndex (' + duration + 'ms)');`,
  },
};
