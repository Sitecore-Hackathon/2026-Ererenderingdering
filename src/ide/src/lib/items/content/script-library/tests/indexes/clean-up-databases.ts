...
export const cleanUpDatabasesTest: ContentItem = {
...
try {
  try {
    ...
    if (!result) throw new Error('cleanUpDatabases returned null');
  } catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>cleanUpDatabases</strong>
...
\`);
print(status + ' cleanUpDatabases (' + duration + 'ms)');`,
  },
};
