...
export const getSupportedLanguagesTest: ContentItem = {
  name: "getSupportedLanguages",
  template: "jsScript",
  fields: {
    "__Display name": "Get Supported Languages",
    Script: `// Test: Languages > getSupportedLanguages
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!langs) throw new Error('supported languages is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getSupportedLanguages</strong>
...
\`);
print(status + ' getSupportedLanguages (' + duration + 'ms)');`,
  },
};
