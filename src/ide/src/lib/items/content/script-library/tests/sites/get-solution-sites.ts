...
export const getSolutionSitesTest: ContentItem = {
  name: "getSolutionSites",
  template: "jsScript",
  fields: {
    "__Display name": "Get Solution Sites",
    Script: `// Test: Sites > getSolutionSites
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!solution) throw new Error('solution sites is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getSolutionSites</strong>
...
\`);
print(status + ' getSolutionSites (' + duration + 'ms)');`,
  },
};
