...
export const createTemplateFolderTest: ContentItem = {
...
try {
  const name = '__sctest_' + Date.now() + '_tmplfolder';
  const folder = await sc.Templates.createTemplateFolder('/sitecore/templates', name);
  try {
    if (!folder) throw new Error('template folder is null');
    if (!folder.itemId) throw new Error('missing itemId');
    if (!folder.name) throw new Error('missing name');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>createTemplateFolder</strong>
...
\`);
print(status + ' createTemplateFolder (' + duration + 'ms)');`,
  },
};
