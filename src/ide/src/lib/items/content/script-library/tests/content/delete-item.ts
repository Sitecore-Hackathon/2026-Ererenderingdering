...
export const deleteItemTest: ContentItem = {
  name: "deleteItem",
  template: "jsScript",
  fields: {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_delete';
  const item = await sc.createItem('/sitecore/content', SAMPLE_TEMPLATE, name);
  if (!item || !item.itemId) throw new Error('setup failed');
  const result = await sc.deleteItem(item.itemId, true);
  if (!result) throw new Error('deleteItem returned null');
  if (result.successful !== true) throw new Error('delete not successful');
  passed = true;
} catch (e) {
...
\`);
print(status + ' deleteItem (' + duration + 'ms)');`,
  },
};
