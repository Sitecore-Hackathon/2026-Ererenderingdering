...
export const getIndexesTest: ContentItem = {
  name: "getIndexes",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (!Array.isArray(indexes)) throw new Error('indexes should be array');
  if (indexes.length === 0) throw new Error('should have indexes');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getIndexes (' + duration + 'ms)');`,
  },
};
