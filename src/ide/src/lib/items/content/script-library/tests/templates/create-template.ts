...
export const createTemplateTest: ContentItem = {
  name: "createTemplate",
  template: "jsScript",
  fields: {
    "__Display name": "Create Template",
    Script: `// Test: Templates > createTemplate
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  const ts = Date.now();
  const folder = await sc.Templates.createTemplateFolder('/sitecore/templates', '__sctest_' + ts + '_tf');
  let templateId = null;
  try {
    if (!folder || !folder.itemId) throw new Error('folder setup failed');
    const tmpl = await sc.Templates.createTemplate(folder.itemId, '__sctest_' + ts + '_tmpl', {
      sections: [{ name: 'Content', fields: [{ name: 'Heading', type: 'Single-Line Text' }] }]
    });
    if (!tmpl) throw new Error('created template is null');
    if (!tmpl.templateId) throw new Error('missing templateId');
    if (!tmpl.name) throw new Error('missing name');
...
\`);
print(status + ' createTemplate (' + duration + 'ms)');`,
  },
};
