...
export const getWorkflowsTest: ContentItem = {
  name: "getWorkflows",
  template: "jsScript",
  fields: {
    "__Display name": "Get Workflows",
    Script: `// Test: Workflows > getWorkflows
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!Array.isArray(wf)) throw new Error('workflows should be array');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getWorkflows (' + duration + 'ms)');`,
  },
};
