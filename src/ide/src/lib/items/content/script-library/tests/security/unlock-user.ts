...
export const unlockUserTest: ContentItem = {
  name: "unlockUser",
  template: "jsScript",
  fields: {
    "__Display name": "Unlock User",
    Script: `// Test: Security > unlockUser
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  try {
    ...
    const result = await sc.Security.unlockUser(userName);
    if (!result) throw new Error('unlock user result is null');
  } finally {
...
\`);
print(status + ' unlockUser (' + duration + 'ms)');`,
  },
};
