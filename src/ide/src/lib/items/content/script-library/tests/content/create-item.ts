...
export const createItemTest: ContentItem = {
  name: "createItem",
  template: "jsScript",
  fields: {
    "__Display name": "Create Item",
    Script: `// Test: Content > createItem
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_create';
  const item = await sc.createItem('/sitecore/content', SAMPLE_TEMPLATE, name, { Title: 'Test' });
  try {
    if (!item) throw new Error('created item is null');
    if (!item.itemId) throw new Error('missing itemId');
    if (!item.name) throw new Error('missing name');
    if (!item.path) throw new Error('missing path');
  } finally {
...
\`);
print(status + ' createItem (' + duration + 'ms)');`,
  },
};
