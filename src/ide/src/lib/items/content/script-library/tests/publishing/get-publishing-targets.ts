...
export const getPublishingTargetsTest: ContentItem = {
...
try {
  ...
  if (!Array.isArray(targets)) throw new Error('publishing targets should be array');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getPublishingTargets</strong>
...
\`);
print(status + ' getPublishingTargets (' + duration + 'ms)');`,
  },
};
