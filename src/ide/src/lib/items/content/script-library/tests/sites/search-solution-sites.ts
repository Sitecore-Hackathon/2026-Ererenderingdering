...
export const searchSolutionSitesTest: ContentItem = {
  name: "searchSolutionSites",
  template: "jsScript",
  fields: {
    "__Display name": "Search Solution Sites",
    Script: `// Test: Sites > searchSolutionSites
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!result) throw new Error('search solution sites is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>searchSolutionSites</strong>
...
\`);
print(status + ' searchSolutionSites (' + duration + 'ms)');`,
  },
};
