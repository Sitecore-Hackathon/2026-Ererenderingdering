...
export const publishSiteTest: ContentItem = {
  name: "publishSite",
  template: "jsScript",
  fields: {
...
try {
  ...
  const result = await sc.Publishing.publishSite({ languages: ['en'], targetDatabases: [targetDb], publishSiteMode: 'SMART' });
  if (!result) throw new Error('publish site result is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' publishSite (' + duration + 'ms)');`,
  },
};
