...
export const getDatabasesTest: ContentItem = {
  name: "getDatabases",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (!Array.isArray(dbs)) throw new Error('databases should be array');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getDatabases (' + duration + 'ms)');`,
  },
};
