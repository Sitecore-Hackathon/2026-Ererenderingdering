...
export const updateItemTest: ContentItem = {
  name: "updateItem",
  template: "jsScript",
  fields: {
    "__Display name": "Update Item",
    Script: `// Test: Content > updateItem
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_update';
  const item = await sc.createItem('/sitecore/content', SAMPLE_TEMPLATE, name);
  try {
    if (!item || !item.itemId) throw new Error('setup failed');
    const updated = await sc.updateItem(item.itemId, { Title: 'Updated' });
    if (!updated) throw new Error('updateItem returned null');
  } finally {
...
\`);
print(status + ' updateItem (' + duration + 'ms)');`,
  },
};
