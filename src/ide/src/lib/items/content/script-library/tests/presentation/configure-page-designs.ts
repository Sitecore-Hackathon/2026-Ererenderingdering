...
export const configurePageDesignsTest: ContentItem = {
...
try {
  ...
  try {
    const result = await sc.Presentation.configurePageDesigns(siteName, []);
    if (!result) throw new Error('configure page designs result is null');
  } catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>configurePageDesigns</strong>
...
\`);
print(status + ' configurePageDesigns (' + duration + 'ms)');`,
  },
};
