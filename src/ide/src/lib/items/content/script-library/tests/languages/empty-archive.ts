...
export const emptyArchiveTest: ContentItem = {
  name: "emptyArchive",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (!result) throw new Error('empty archive result is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' emptyArchive (' + duration + 'ms)');`,
  },
};
