...
export const getArchivedItemsTest: ContentItem = {
  name: "getArchivedItems",
  template: "jsScript",
  fields: {
    "__Display name": "Get Archived Items",
    Script: `// Test: Languages > getArchivedItems
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!archived) throw new Error('archived items result is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getArchivedItems</strong>
...
\`);
print(status + ' getArchivedItems (' + duration + 'ms)');`,
  },
};
