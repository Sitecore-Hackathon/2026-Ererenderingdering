...
export const getSiteTest: ContentItem = {
  name: "getSite",
  template: "jsScript",
  fields: {
    "__Display name": "Get Site",
    Script: `// Test: Sites > getSite
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  const site = await sc.Sites.getSite(siteName);
...
\`);
print(status + ' getSite (' + duration + 'ms)');`,
  },
};
