...
export const getTemplatesTest: ContentItem = {
  name: "getTemplates",
  template: "jsScript",
  fields: {
    "__Display name": "Get Templates",
    Script: `// Test: Templates > getTemplates
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (templates.length === 0) throw new Error('should have templates');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getTemplates (' + duration + 'ms)');`,
  },
};
