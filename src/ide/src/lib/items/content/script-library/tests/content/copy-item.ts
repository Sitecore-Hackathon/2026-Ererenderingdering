...
export const copyItemTest: ContentItem = {
  name: "copyItem",
  template: "jsScript",
  fields: {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_copy';
  const item = await sc.createItem('/sitecore/content', SAMPLE_TEMPLATE, name);
  let copyId = null;
  try {
    if (!item || !item.itemId) throw new Error('setup failed');
    const copy = await sc.copyItem(item.itemId, '/sitecore/content', { copyItemName: name + '_cp' });
    if (!copy) throw new Error('copyItem returned null');
    if (!copy.itemId) throw new Error('copy missing itemId');
    copyId = copy.itemId;
  } finally {
    try { if (copyId) await sc.deleteItem(copyId, true); } catch(e) { /* cleanup */ }
...
\`);
print(status + ' copyItem (' + duration + 'ms)');`,
  },
};
