...
export const getSiteCollectionsTest: ContentItem = {
  name: "getSiteCollections",
  template: "jsScript",
  fields: {
    "__Display name": "Get Site Collections",
    Script: `// Test: Sites > getSiteCollections
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!collections) throw new Error('site collections is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getSiteCollections</strong>
...
\`);
print(status + ' getSiteCollections (' + duration + 'ms)');`,
  },
};
