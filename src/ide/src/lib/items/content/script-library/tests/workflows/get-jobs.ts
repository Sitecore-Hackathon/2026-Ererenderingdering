...
export const getJobsTest: ContentItem = {
  name: "getJobs",
  template: "jsScript",
  fields: {
    "__Display name": "Get Jobs",
    Script: `// Test: Workflows > getJobs
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!jobs) throw new Error('jobs result is null');
  passed = true;
} catch (e) {
...
\`);
print(status + ' getJobs (' + duration + 'ms)');`,
  },
};
