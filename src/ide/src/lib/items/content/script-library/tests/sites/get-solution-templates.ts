...
export const getSolutionTemplatesTest: ContentItem = {
  name: "getSolutionTemplates",
  template: "jsScript",
  fields: {
    "__Display name": "Get Solution Templates",
    Script: `// Test: Sites > getSolutionTemplates
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!templates) throw new Error('solution templates is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>getSolutionTemplates</strong>
...
\`);
print(status + ' getSolutionTemplates (' + duration + 'ms)');`,
  },
};
