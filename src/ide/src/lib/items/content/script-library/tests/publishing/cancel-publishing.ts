...
export const cancelPublishingTest: ContentItem = {
...
try {
  try {
    const result = await sc.Publishing.cancelPublishing('nonexistent');
    if (!result) throw new Error('cancel publishing result is null');
  } catch (e) {
    // Canceling a completed or invalid operation may throw - that's OK
    if (!e || !e.message) throw new Error('cancel threw without message');
  }
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>cancelPublishing</strong>
...
\`);
print(status + ' cancelPublishing (' + duration + 'ms)');`,
  },
};
