...
export const changeUserPasswordTest: ContentItem = {
  name: "changeUserPassword",
  template: "jsScript",
  fields: {
    "__Display name": "Change User Password",
    Script: `// Test: Security > changeUserPassword
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  try {
    ...
    const result = await sc.Security.changeUserPassword(userName, 'T3st!Pass' + ts, 'N3w!Pass' + ts);
    if (!result) throw new Error('change password result is null');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>changeUserPassword</strong>
...
\`);
print(status + ' changeUserPassword (' + duration + 'ms)');`,
  },
};
