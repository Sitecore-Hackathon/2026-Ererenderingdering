...
export const rebuildLinkDatabaseTest: ContentItem = {
...
try {
  try {
    ...
    if (!result) throw new Error('rebuildLinkDatabase returned null');
  } catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>rebuildLinkDatabase</strong>
...
\`);
print(status + ' rebuildLinkDatabase (' + duration + 'ms)');`,
  },
};
