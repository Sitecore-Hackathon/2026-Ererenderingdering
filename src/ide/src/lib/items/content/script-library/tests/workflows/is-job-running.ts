...
export const isJobRunningTest: ContentItem = {
  name: "isJobRunning",
  template: "jsScript",
  fields: {
...
try {
  ...
  if (result !== false) throw new Error('nonexistent job should not be running');
  passed = true;
} catch (e) {
...
\`);
print(status + ' isJobRunning (' + duration + 'ms)');`,
  },
};
