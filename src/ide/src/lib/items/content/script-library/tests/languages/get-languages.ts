...
export const getLanguagesTest: ContentItem = {
  name: "getLanguages",
  template: "jsScript",
  fields: {
    "__Display name": "Get Languages",
    Script: `// Test: Languages > getLanguages
const t0 = Date.now();
let passed = false;
let error = null;
...
try {
  ...
  if (!Array.isArray(langs)) throw new Error('languages should be array');
...
\`);
print(status + ' getLanguages (' + duration + 'ms)');`,
  },
};
