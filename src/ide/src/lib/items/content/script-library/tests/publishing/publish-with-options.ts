...
export const publishWithOptionsTest: ContentItem = {
...
try {
  ...
  const result = await sc.Publishing.publishWithOptions([{ language: 'en', sourceDatabase: 'master', targetDatabase: targetDb, publishSiteMode: 'SMART' }]);
  if (!result) throw new Error('publish with options result is null');
  passed = true;
} catch (e) {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>publishWithOptions</strong>
...
\`);
print(status + ' publishWithOptions (' + duration + 'ms)');`,
  },
};
