...
export const setItemArchiveDateTest: ContentItem = {
...
try {
  ...
  const name = '__sctest_' + Date.now() + '_archdate';
  ...
  try {
    ...
    const result = await sc.Languages.setItemArchiveDate(item.itemId, futureDate);
    if (!result) throw new Error('set item archive date result is null');
  } finally {
...
<div style="font-family:var(--font-sans),system-ui,sans-serif;padding:16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
    ...
    <strong>setItemArchiveDate</strong>
...
\`);
print(status + ' setItemArchiveDate (' + duration + 'ms)');`,
  },
};
