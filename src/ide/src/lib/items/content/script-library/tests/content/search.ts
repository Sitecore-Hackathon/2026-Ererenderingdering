...
export const searchTest: ContentItem = {
  name: "search",
  template: "jsScript",
  fields: {
...
try {
  const result = await sc.search({
    searchStatement: { criteria: [{ field: '_name', value: 'content', criteriaType: 'CONTAINS' }] },
    paging: { pageSize: 5 }
  });
  if (!result) throw new Error('search result is null');
  if (!('totalCount' in result)) throw new Error('missing totalCount');
  passed = true;
} catch (e) {
...
\`);
print(status + ' search (' + duration + 'ms)');`,
  },
};
